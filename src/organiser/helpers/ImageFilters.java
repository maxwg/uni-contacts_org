...
public class ImageFilters {
	private static final BoxBlurFilter RecordBlurFilter = new BoxBlurFilter(50,
			50, 1);
	...
	private static GlowFilter RecordGlowFilter2;
	static {
		RecordGlowFilter2 = new GlowFilter();
		RecordGlowFilter2.setAmount(0.25f);
	}
	...
	public static BufferedImage resizeImage(BufferedImage img, int maxSizeTo) {
		...
		int rsHeight = img.getWidth() > img.getHeight() ? maxSizeTo : maxSizeTo
	...
	public static BufferedImage recordDarken(BufferedImage image,
			boolean selected) {
		if (selected)
			RecordGlowFilter2.filter(image, image);
		else
